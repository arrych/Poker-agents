...
def extract_action_from_text(text):
    ...
    if match:
        action = match.group(1).strip()  # 提取匹配到的动作并去除两端的空白字符
        temp = actions.get(action)
        if temp is not None:  
            return temp
    return 2  # 如果没有找到匹配的动作，返回None
...
# 比较两手牌的结果
def compare_hands(player_hand, opponent_hand, community_card):
...
def analyse(player_hand, community_card):
    ...
    for opponent_hand in remaining_cards:
        result = compare_hands(player_hand, opponent_hand, community_card)
        results[result] += 1
        output_lines.append(
