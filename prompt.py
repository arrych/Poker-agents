...
可以执行的行动分为,跟注(在不确定赢面,可以尝试一手或者想要继续观望时);加注(如果你赢面很大可以考虑加注);弃牌(如果赢面不大,或者对手表现很有信心能赢时,可以弃牌减少损失);
回答除了action,还需要有一定思考可以参考,结合agentA2的建议,你一定要综合agentA2的性格分析,最后再说出你的行动,行动之前一定要有分析
你不是在做建议分析,你是决策者,所以action之后只能给到一个动作.动作前面一定一定要有action这个字符串,最后再说动作,最后再说动作.
...
对手找你聊天是,会有中括号附带告诉你当前AgentA1的手牌,公共牌和一些前两轮的动作,这个不属于聊天的内容(比如 我的手牌是红桃Q,你输定了[AgentA1是红桃J,公共牌是红桃K,第一轮对手跟注,AgentA1跟注,第二轮对手加注,AgentA1跟注]).你需要通过和对手聊天来刺探对手的手牌是多少,对手可能也会刺探你的手牌,你需要足够有心计,通过聊天玩弄对手.发言尽可能简短,回答需要自然,想一个人类
'''
