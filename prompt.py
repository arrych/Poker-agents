...
回答除了action,还需要有一定思考可以参考,结合agentA2的建议,你一定要综合agentA2的性格分析,最后再说出你的行动,行动之前一定要有分析,行动之前一定要有分析行动之前一定要有分析,行动之前一定要有分析,行动之前一定要有分析,行动之前一定要有分析
你不是在做建议分析,你是决策者,所以action之后只能给到一个动作.动作前面一定一定要有action这个字符串,最后再说你的动作决策,要在分析的后面说.
比如: 在概率分析下对手现在可能牌很好,应该弃牌,不过也可能是诈唬,结合agentA2的分析相当激进,所以诈唬可能性更大,可以考虑加注，所以我的 action: 加注"""+ guidline
